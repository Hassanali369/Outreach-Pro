...
export default function App() {
  ...
  const handleLogout = async () => {
    try {
      if (sendingStatus === 'running') {
        if (!confirm("Campaign is running. If you log out, the campaign will pause. Continue?")) return;
        pauseCampaign();
      }
      await signOut(auth);
    } catch (error) {
  ...
  const [unsubscribeWorkspace, setUnsubscribeWorkspace] = useState<string | null>(() => {
    if (typeof window !== 'undefined') {
      const params = new URLSearchParams(window.location.search);
      return params.get('w');
    }
  ...
  useEffect(() => {
    if (unsubscribeEmail && unsubscribeStatus === 'loading') {
      addDoc(collection(db, 'unsubscribes'), {
        ...
        workspaceId: unsubscribeWorkspace || 'legacy',
        timestamp: Date.now()
      }).then(() => {
  ...
  const [workspaceId, setWorkspaceId] = useState<string>(() => {
    const saved = localStorage.getItem('outreach_workspace_id');
    if (saved) return saved;
    const newId = Math.random().toString(36).substring(2, 10).toUpperCase();
    localStorage.setItem('outreach_workspace_id', newId);
    return newId;
  });

  useEffect(() => {
    localStorage.setItem('outreach_workspace_id', workspaceId);
  }, [workspaceId]);

  useEffect(() => {
    if (user?.uid && localStorage.getItem('outreach_workspace_id_linked') !== user.uid) {
      setWorkspaceId(user.uid);
      localStorage.setItem('outreach_workspace_id_linked', user.uid);
    }
  }, [user]);
  ...
  useEffect(() => {
    ...
    const q = query(collection(db, 'unsubscribes'), where('workspaceId', '==', workspaceId));
  ...
  }, [sendingStatus, currentIndex, accounts, contacts, subjects, bodies, delayType, delaySeconds, delayMin, delayMax, combinedBlacklist, includeUnsubscribe, workspaceId]);
    ...
    if (toEmail && subject && html) {
      try {
        ...
        if (state.includeUnsubscribe) {
          ...
          const unsubLink = `${baseUrl}/?unsubscribe=${encodeURIComponent(toEmail)}&w=${encodeURIComponent(state.workspaceId)}`;
  ...
  if (authLoading) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4">
        ...
        <p className="mt-4 text-slate-500 font-medium">Checking authentication...</p>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-2xl shadow-sm border border-slate-200 p-8 text-center animate-in fade-in zoom-in duration-500">
          ...
          <h1 className="text-2xl font-bold text-slate-900 mb-2">Welcome to OutreachPro</h1>
          <p className="text-slate-500 mb-8">Sign in with your Google account to access your campaigns and manage your personal blacklist securely.</p>
          
          <button 
            onClick={handleLogin}
            className="w-full flex items-center justify-center gap-3 bg-white border border-slate-300 text-slate-700 hover:bg-slate-50 font-medium py-3 px-4 rounded-xl shadow-sm transition-all focus:ring-2 focus:ring-slate-200 outline-none"
          >
            <svg viewBox="0 0 24 24" className="w-5 h-5" xmlns="http://www.w3.org/2000/svg"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.16v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.16C1.43 8.55 1 10.22 1 12s.43 3.45 1.16 4.93l2.85-2.22.83-.62z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.16 7.07l3.68 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/></svg>
            Sign in with Google
          </button>
        </div>
      </div>
      ...
      <aside className={`${isSidebarCollapsed ? 'w-20' : 'w-64'} bg-slate-900 text-slate-300 flex flex-col shadow-xl z-20 transition-all duration-300 shrink-0`}>
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-950/50">
          <div className={`flex items-center gap-3 mb-4 ${isSidebarCollapsed ? 'justify-center' : ''}`}>
            {user?.photoURL ? (
              <img src={user.photoURL} alt="User avatar" className="w-8 h-8 rounded-full border border-slate-700 shrink-0" />
            ) : (
              <div className="w-8 h-8 rounded-full bg-indigo-500/20 text-indigo-400 border border-indigo-500/30 font-medium flex items-center justify-center shrink-0">
          ...
          <button 
            ...
            className={`w-full flex items-center gap-2 text-slate-400 hover:text-white hover:bg-slate-800 py-2 rounded-lg transition-colors ${isSidebarCollapsed ? 'justify-center px-0' : 'px-3'}`}
            title="Sign Out"
          >
            ...
            {activeTab === 'settings' && (
              <div className="bg-white rounded-xl shadow-sm border border-slate-200 max-w-2xl animate-in fade-in slide-in-from-bottom-4 duration-500 overflow-hidden">
                ...
                <div className="p-6 space-y-8">
                  ...
                  <div className="border-t border-slate-200 pt-8">
                    <label className="block text-sm font-semibold text-slate-900 mb-2">Team / Workspace ID</label>
                    <p className="text-sm text-slate-500 mb-4">Use this ID to share your Unsubscribe Blacklist with team members. If you paste this ID on another computer, anyone who unsubscribes will show up in both places.</p>
                    <div className="flex items-center gap-3 max-w-sm">
                      <input 
                        type="text" 
                        value={workspaceId} 
                        onChange={e => setWorkspaceId(e.target.value.toUpperCase().trim())} 
                        className="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none text-sm shadow-sm uppercase font-mono" 
                      />
                      <button 
                        onClick={() => navigator.clipboard.writeText(workspaceId)} 
                        className="px-4 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-sm font-medium rounded-lg border border-slate-200 transition-colors shrink-0"
                      >
                        Copy
                      </button>
                    </div>
